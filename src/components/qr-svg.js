...
export function SAFEQRCode({
    ...
    const cornersRect = corners.map((l) => (
      <rect x={l.x} y={l.y}
        rx={15} ry={15}
        height={diameter * (cornerLength-1)}
        width={diameter * (cornerLength-1)}
        style={{ stroke: fgColor, strokeWidth: radius, fill: 'none' }}
      />
    ));

    const cornersCircle = corners.map((l) => (
      <circle
        cx={diameter * 3 + l.x}
        cy={diameter * 3 + l.y}
        r={3 * radius}
    ...
    return (<svg shapeRendering="crispEdges" viewBox={viewBoxSizes.join(' ')} {...otherProps}>
        {
            cells.map((row, rowIndex) =>
                row.map((cell, colIndex) => {
                  if /*(((rowIndex === 3) || (rowIndex === 8)) && ((colIndex === 7) || (colIndex === 13))) {
                    return (
                      <line x1={diameter * colIndex}
                        y1={diameter * rowIndex}
                        x2={diameter * (colIndex - 1)}
                        y2={diameter * (rowIndex + 1)}
                        style={{ stroke: fgColor, strokeWidth: Math.floor(0.7 * radius)}}
                      />);
                  } else if*/ ((rowIndex <= logoStarts || rowIndex > logoEnds || colIndex < logoStarts)
